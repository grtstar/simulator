...
def mapping_odom2(odom : mars_message.Odometry, frame_id):
    ...
    #(trans, rot) = tf2_ros.transformer.lookupTransform("map", "base_link", rospy.Time.now())
    ...
    # trans.header.frame_id = "bask_link"
    ...
    # trans2 = geometry_msgs.msg.TransformStamped()  # 定义广播者要广播的数据类型
    # trans2.header.stamp = rospy.Time.now()
    # trans2.header.frame_id = "map"
    # trans2.child_frame_id = "base_link"map
    # trans2.transform.translation.x = odom.pose.pose.x
    # trans2.transform.translation.y = odom.pose.pose.y
    # trans2.transform.translation.z = odom.pose.pose.z
	# # 从偏航角解算出对应的四元数，只考虑平面运动
    # trans2.transform.rotation.x = odom.pose.orientation.x
    # trans2.transform.rotation.y = odom.pose.orientation.y
    # trans2.transform.rotation.z = odom.pose.orientation.z
    # trans2.transform.rotation.w = odom.pose.orientation.w
    # br.sendTransform(trans2)
...
lcm_mapping = [
    LcmMapping("RobotSpeed", "/c1_drive_controller/cmd_vel",mars_message.Twist, geometry_msgs.msg.Twist, "base_link", mapping_speed),
    ...
    LcmMapping("OBS", "OBS", mars_message.PointCloud, sensor_msgs.msg.PointCloud, "body", mapping_pointcloud),
    ...
    #LcmMapping("BUMPERCLIFF", "BUMPERCLIFF",mars_message.PointCloud, sensor_msgs.msg.PointCloud, "imu_link", mapping_pointcloud),
    #LcmMapping("BUMPERCLIFF", "BUMPERCLIFF",mars_message.PointCloud, sensor_msgs.msg.PointCloud, "imu_link", mapping_pointcloud),
    ...
    #LcmMapping("REALODOM", mars_message.Odometry, geometry_msgs.msg.PoseStamped, "realodom", mapping_odom2),
    LcmMapping("MAP", "MAP",mars_message.GridMap, nav_msgs.msg.OccupancyGrid, "odom", mapping_map),
    LcmMapping("NAVIMAP", "NAVIMAP", mars_message.GridMap, nav_msgs.msg.OccupancyGrid, "odom", mapping_map),
    LcmMapping("LOCALMAP", "LOCALMAP",mars_message.GridMap, nav_msgs.msg.OccupancyGrid, "odom", mapping_map),
    LcmMapping("REFINE", "REFINE",mars_message.GridMap, nav_msgs.msg.OccupancyGrid, "odom", mapping_map),
    LcmMapping("UNCLEAN", "UNCLEAN",mars_message.GridMap, nav_msgs.msg.OccupancyGrid, "odom", mapping_map),
    LcmMapping("CLEANED", "CLEANED",mars_message.GridMap, nav_msgs.msg.OccupancyGrid, "odom", mapping_map),
    LcmMapping("AUTO_ROM_MAP", "AUTO_ROM_MAP",mars_message.GridMap, nav_msgs.msg.OccupancyGrid, "odom", mapping_map),
    LcmMapping("FIND_ROM_MAP", "FIND_ROM_MAP",mars_message.GridMap, nav_msgs.msg.OccupancyGrid, "odom", mapping_map),
    ...
    LcmMapping("RECT", "RECT",mars_message.Marker, visualization_msgs.msg.Marker, "odom", mapping_marker),
    LcmMapping("BLOCK", "BLOCK",mars_message.Marker, visualization_msgs.msg.Marker, "odom", mapping_marker),
    LcmMapping("slam_path", "slam_path",mars_message.Path, nav_msgs.msg.Path, "odom", mapping_path),
    LcmMapping("wheel_path", "wheel_path",mars_message.Path, nav_msgs.msg.Path, "odom", mapping_path),
]
...
def lcm_message_handler(channel, data):
    ...
    lcm_pubs[channel].publish(ros_msg)
